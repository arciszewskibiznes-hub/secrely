...
export default function MessagesPage() {
  ...
  return (
    <div className="flex gap-4 h-[calc(100vh-9rem)]">
      <div className={cn("flex flex-col gap-2 w-full md:w-72 flex-shrink-0", activeConv && "hidden md:flex")}>
        ...
        <div className="flex-1 overflow-y-auto space-y-1 pr-1">
          {MOCK_CONVERSATIONS.map((conv) => (
            <motion.button
              ...
              className={cn("w-full flex items-center gap-3 p-3 rounded-xl text-left transition-all hover:bg-secondary", activeConv === conv.id && "bg-purple-50 hover:bg-purple-50")}
            >
              <div className="relative flex-shrink-0">
                ...
                {conv.isOnline && <Circle className="absolute bottom-0 right-0 w-3 h-3 fill-emerald-500 text-emerald-500 bg-white rounded-full" />}
              </div>
              ...
              {conv.unreadCount > 0 && (
                <span className="w-5 h-5 flex-shrink-0 bg-[hsl(270,75%,60%)] text-white text-[10px] font-bold rounded-full flex items-center justify-center">{conv.unreadCount}</span>
              )}
            </motion.button>
          ))}
        </div>
      ...
      {activeConv && activeConversation ? (
        <div className={cn("flex-1 flex flex-col card-base overflow-hidden", "flex")}>
          <div className="flex items-center gap-3 px-4 py-3 border-b border-border">
            <button onClick={() => setActiveConv(null)} className="md:hidden text-muted-foreground hover:text-foreground text-sm">{t.back}</button>
            ...
            <div>
              ...
              <div className="text-xs text-muted-foreground flex items-center gap-1">
                {activeConversation.isOnline ? (
                  <><Circle className="w-2 h-2 fill-emerald-500 text-emerald-500" /> {t.online}</>
                ) : t.offline}
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {messages.length === 0 ? (
              <EmptyState
                ...
                title={t.startConv}
                description={interpolate(t.startDesc, { name: activeConversation.participant.displayName })}
              />
            ) : (
              messages.map((msg) => {
                ...
                return (
                  <div key={msg.id} className={cn("flex gap-2", isMe && "flex-row-reverse")}>
                    ...
                    <div className={cn("max-w-[75%] px-3.5 py-2.5 rounded-2xl text-sm leading-relaxed", isMe ? "bg-[hsl(270,75%,60%)] text-white rounded-tr-sm" : "bg-secondary text-foreground rounded-tl-sm")}>
                      {msg.content}
                    </div>
                  </div>
                );
          ...
          <div className="p-3 border-t border-border flex gap-2">
            <Input
              placeholder={interpolate(t.placeholder, { name: activeConversation.participant.displayName })}
              value={message}
