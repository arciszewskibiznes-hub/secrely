      ...
      <div className="grid grid-cols-4 gap-2">
        {(["all", "pending", "approved", "rejected"] as Filter[]).map((f) => (
          <button key={f} onClick={() => setFilter(f)}
            className={cn("card-base p-3 text-center transition-all",
              filter === f && f !== "all" && `${STATUS_CONFIG[f as keyof typeof STATUS_CONFIG]?.border} border-2`,
              filter === f && f === "all" && "border-[hsl(270,75%,60%)] border-2")}>
            <div className="text-lg font-bold text-foreground tabular-nums">{counts[f]}</div>
            <div className="text-[10px] text-muted-foreground capitalize mt-0.5">{f === "all" ? t.all : t[f as "approved" | "rejected"] ?? f}</div>
          </button>
        ))}
      </div>
        ...
        {filtered.map((item, i) => {
          ...
          return (
            <motion.div key={item.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="card-base overflow-hidden">
              ...
              {isExpanded && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} transition={{ duration: 0.25 }} className="border-t border-border px-4 py-3 bg-secondary/40 space-y-3">
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    <div><span className="text-muted-foreground block mb-0.5">{t.detailCreator}</span><span className="font-medium text-foreground">{item.creator.displayName}</span></div>
                    <div><span className="text-muted-foreground block mb-0.5">{t.detailSubscribers}</span><span className="font-medium text-foreground">{item.creator.subscriberCount.toLocaleString()}</span></div>
                    <div><span className="text-muted-foreground block mb-0.5">{t.detailType}</span><span className="font-medium text-foreground capitalize">{item.type}</span></div>
                    <div><span className="text-muted-foreground block mb-0.5">{t.detailVerified}</span><span className={cn("font-medium", item.creator.isVerified ? "text-emerald-600" : "text-muted-foreground")}>{item.creator.isVerified ? t.yes : t.no}</span></div>
                  </div>
                  <div><span className="text-xs text-muted-foreground block mb-1">{t.fullDesc}</span><p className="text-xs text-foreground leading-relaxed">{item.content}</p></div>
                </motion.div>
              )}
            </motion.div>
          );
