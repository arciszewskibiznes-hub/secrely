...
export default function WalletPage() {
  ...
  const handlePurchase = (credits: number, bonus: number | undefined, label: string) => {
    ...
    addCredits(total, interpolate(t.purchased, { label }));
    toast({
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {CREDIT_PACKAGES.map((pkg, i) => (
            ...
            >
              ...
              <Button size="sm" variant={pkg.popular ? "purple" : "outline"} className="w-full text-xs h-8"
        ...
        <p className="text-xs text-muted-foreground text-center mt-3">{t.demoNote}</p>
      </div>
          ...
          {transactions.map((tx) => (
            <div key={tx.id} className="flex items-center gap-3 px-4 py-3">
              <div className={cn("w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0", tx.amount > 0 ? "bg-emerald-50" : "bg-red-50")}>
                {tx.amount > 0 ? <ArrowUpRight className="w-4 h-4 text-emerald-600" /> : <ArrowDownLeft className="w-4 h-4 text-rose-500" />}
              </div>
