    ...
    <div className="space-y-5">
      ...
      >
        <div className="flex items-end justify-between mb-4">
          ...
          <Button
            variant={subscribed ? "outline" : "purple"}
            onClick={() => subscribed ? unsubscribe(creator.id) : subscribe(creator.id)}
          >
        ...
        <div className="grid grid-cols-3 gap-3 mt-4 pt-4 border-t border-border">
          {[
            { icon: Users, value: formatNumber(creator.subscriberCount), label: "Followers" },
            { icon: ImageIcon, value: creator.postCount, label: "Posts" },
            { icon: TrendingUp, value: formatNumber(creator.stats.totalLikes), label: "Likes" },
        ...
        {lockedCount > 0 && !subscribed && (
          <div className="mt-4 p-3 bg-purple-50 rounded-xl border border-purple-100 flex items-center gap-3">
            ...
            <div>
              <div className="text-xs font-semibold text-purple-800">{lockedCount} premium posts available</div>
              <div className="text-xs text-purple-600">Unlock each post individually with credits</div>
            </div>
          </div>
        )}
      </motion.div>
      ...
      <div className="space-y-4">
        <h2 className="text-sm font-semibold text-foreground">{posts.length} Posts</h2>
        {posts.map((post) => (
