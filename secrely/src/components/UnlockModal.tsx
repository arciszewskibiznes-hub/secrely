...
export function UnlockModal({ open, onClose, post }: UnlockModalProps) {
  ...
  const handleUnlock = async () => {
    ...
    const success = spendCredits(post.price, `${post.teaserText.slice(0, 60)} — ${post.creator.displayName}`, post.id);
    setIsLoading(false);
    if (success) {
      unlockPost(post.id);
      setStep("success");
      toast({ title: t.successTitle + " 🎉", description: interpolate(t.successDesc, { name: post.creator.displayName }), variant: "success" });
    } else {
      ...
      <DialogContent className="max-w-sm">
        {step === "confirm" && (
          <>
            <DialogHeader>
              ...
              <DialogDescription className="text-center">{interpolate(t.description, { name: post.creator.displayName })}</DialogDescription>
            </DialogHeader>
            ...
            <p className="text-sm text-muted-foreground text-center leading-relaxed">{post.teaserText}</p>

            <div className="bg-purple-50 rounded-xl p-4 space-y-2">
              <div className="flex items-center justify-between text-sm">
                <span className="text-muted-foreground">{t.unlockPrice}</span>
                <span className="font-semibold flex items-center gap-1"><Coins className="w-3.5 h-3.5 text-[hsl(270,75%,60%)]" />{post.price} credits</span>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-muted-foreground">{t.yourBalance}</span>
                <span className={`font-semibold ${canAfford ? "text-emerald-600" : "text-rose-500"}`}>{formatCredits(balance)} credits</span>
              </div>
              {canAfford && (
                <div className="flex items-center justify-between text-sm pt-1 border-t border-purple-200">
                  ...
                  <span className="font-semibold text-foreground">{formatCredits(balance - post.price)} credits</span>
                </div>
              )}
            </div>
            ...
            <div className="flex gap-3">
              ...
              {canAfford ? (
                <Button variant="purple" className="flex-1" onClick={handleUnlock} disabled={isLoading}>
                  {isLoading ? t.unlocking : interpolate(t.unlockFor, { price: post.price })}
                </Button>
              ) : (
                <Button variant="default" className="flex-1" onClick={handleClose} asChild>
                  <a href="/wallet">{t.getCredits}</a>
                </Button>
              )}
            </div>
          </>
        ...
        {step === "success" && (
          <div className="flex flex-col items-center gap-4 py-4">
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-muted-foreground mt-1">{interpolate(t.successDesc, { name: post.creator.displayName })}</p>
            </div>
            <Button variant="purple" className="w-full" onClick={handleClose}>{t.viewContent}</Button>
          </div>
        )}
        ...
        {step === "insufficient" && (
          <div className="flex flex-col items-center gap-4 py-4">
            ...
            <div className="text-center">
              <h3 className="text-lg font-semibold">{t.insufficientTitle}</h3>
              <p className="text-sm text-muted-foreground mt-1">{t.insufficientDesc}</p>
            </div>
            <Button variant="purple" className="w-full" onClick={handleClose} asChild>
              <a href="/wallet">{t.topUp}</a>
            </Button>
          </div>
        )}
      </DialogContent>
